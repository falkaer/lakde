...
def expect_log_q_lambda(nu, w, expect_log_lambda):
    return torch.sum(
        -lgamma(nu) + nu * torch.log(w) + (nu - 1) * expect_log_lambda - nu
    )
...
class SharedDiagonalizedKDE(AbstractKDE):
    ...
    def log_pred_density(self, X, Y):
        ...
        for bi, i in enumerate(range(0, N_train, bsize)):
            ...
            for j in range(0, N_test, bsize):
                Y_test = Y[j : j + bsize]
                log1p_Q = (
                    (Y_test[None, :] - X_train[:, None]).pow_(2).mul_(inv_w).log1p_()
                )
    ...
    def sample(self, X, n):
        ...
        u = torch.clamp_min(
            torch.distributions.Chi2(df=2 * self.nu).sample((n, D)), eps
        )
